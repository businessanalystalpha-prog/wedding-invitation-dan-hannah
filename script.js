...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im16cXVoa3F5Z2djdHl4cWZwcHFoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODcxMjk2OTksImV4cCI62102cwNTY5OX0.a7OKi1yDaxjPcoUpFplJgHPpwRSRtQb0YSgxSsNGRVY';
...
// --- Audio & Universal Video Auto-Pause/Resume Logic ---
const audio = document.getElementById('wedding-audio');
...
// 1. Local Video Event Listeners
const localVideo = document.querySelector('#save-the-date video');
if (localVideo) {
  localVideo.addEventListener('play', pauseMusicForVideo);
  localVideo.addEventListener('pause', resumeMusicAfterVideo);
  localVideo.addEventListener('ended', resumeMusicAfterVideo);
}

// 2. YouTube Embed Event Listeners via YouTube Iframe API
const ytIframe = document.getElementById('yt-player') || document.querySelector('#save-the-date iframe');
if (ytIframe) {
  let src = ytIframe.getAttribute('src');
  if (src && !src.includes('enablejsapi=1')) {
    ytIframe.setAttribute('src', src + (src.includes('?') ? '&' : '?') + 'enablejsapi=1');
  }
  if (!ytIframe.id) {
    ytIframe.id = 'yt-player';
  }
